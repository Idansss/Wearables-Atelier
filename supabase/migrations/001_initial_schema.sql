-- ============================================================
--  Wearables Atelier — Full Supabase Migration
...
-- ─── ADMINS (must be created first — other policies reference admins) ─
...
-- Only admins can write (checked via is_admin() RPC defined below)
...
-- Anyone can submit a custom order
create policy "custom_orders_public_insert" on custom_orders
  for insert with check (true);

-- Only admins can read / update
create policy "custom_orders_admin_read" on custom_orders
  for select using (
    exists (select 1 from admins where email = (select email from auth.users where id = auth.uid()) and is_active = true)
  );

create policy "custom_orders_admin_update" on custom_orders
  for update using (
    exists (select 1 from admins where email = (select email from auth.users where id = auth.uid()) and is_active = true)
  );
...
-- Public can subscribe (via RPC below — direct insert blocked)
...
create policy "wholesale_public_insert" on wholesale_leads
  for insert with check (true);

create policy "wholesale_admin_read" on wholesale_leads
  for select using (
    exists (select 1 from admins where email = (select email from auth.users where id = auth.uid()) and is_active = true)
  );

create policy "wholesale_admin_update" on wholesale_leads
  for update using (
    exists (select 1 from admins where email = (select email from auth.users where id = auth.uid()) and is_active = true)
  );
...
-- Public can read active coupons (needed for validateCoupon at checkout)
...
-- Public can read site settings (needed for navbar, footer, shop)
...
-- exists in the admins table with is_active = true.
...
-- ─── subscribe_newsletter(p_email, p_source) ─────────────────
-- Upserts a newsletter subscriber. Returns the subscriber id.
...
-- ─── increment_coupon_use(p_id) ──────────────────────────────
-- Atomically increments the used_count on a coupon row.
...
-- ============================================================
--  SEED: Insert your first admin so you can log in
--  Replace the email below with YOUR Supabase auth email.
-- ============================================================
